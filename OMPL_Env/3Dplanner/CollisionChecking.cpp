...
bool isValidPoint(double x, double y, const std::vector<Rectangle> &obstacles)
{
    for (const auto& rect : obstacles)
    {
        ...
        if (x >= rect.x && x <= (rect.x + rect.length) &&
            y >= (rect.y) && y <= (rect.y + rect.breadth))
        {
            std::cout << x << rect.x << rect.length << y << rect.y << rect.breadth << std::endl;
...
bool CheckPointInside(double x, double y, Rectangle r){
    return x>=r.x && x<=r.x+r.length && y>=r.y && y<=r.y+r.breadth;
}
...
bool isValidSquare(double x, double y, double z, double theta, double sideLength, const std::vector<Rectangle> &obstacles)
{
    ...
    rx1.push_back(c * (-(sideLength + threshold)/2.0) - s * (-(sideLength + threshold)/2.0) + x);
    ry1.push_back(s * (-(sideLength + threshold)/2.0) + c * (-(sideLength + threshold)/2.0) + y); 
    rx2.push_back(c * ((sideLength + threshold)/2.0) - s * (-(sideLength + threshold)/2.0) + x);
    ry2.push_back(s * ((sideLength + threshold)/2.0) + c * (-(sideLength + threshold)/2.0) + y);
    ...
    for (int i = 0; i < (int)obstacles.size(); i++) {
        if (z < obstacles[i].height + threshold)
        {
            for (int j = 0; j < (int)rx1.size(); j++){
                // Get the 4 line segments of the obstacle and check intersections with the transformed object
                if(CheckSegmentsIntersect(rx1[j], ry1[j], rx2[j], ry2[j], obstacles[i].x, obstacles[i].y, obstacles[i].x+obstacles[i].length, obstacles[i].y)){
                    return false;
                } else if(CheckSegmentsIntersect(rx1[j], ry1[j], rx2[j], ry2[j], obstacles[i].x, obstacles[i].y, obstacles[i].x, obstacles[i].y+obstacles[i].breadth)){
                    return false;
                } else if(CheckSegmentsIntersect(rx1[j], ry1[j], rx2[j], ry2[j], obstacles[i].x+obstacles[i].length, obstacles[i].y+obstacles[i].breadth, obstacles[i].x+obstacles[i].length, obstacles[i].y)){
                    return false;
                } else if(CheckSegmentsIntersect(rx1[j], ry1[j], rx2[j], ry2[j], obstacles[i].x+obstacles[i].length, obstacles[i].y+obstacles[i].breadth, obstacles[i].x, obstacles[i].y+obstacles[i].breadth)){
                    return false;
